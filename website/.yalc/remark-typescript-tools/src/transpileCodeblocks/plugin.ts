...
export const transpileCodeblocks: Plugin<[TranspileCodeblocksSettings]> =
  function ({
    ...
    if (!compilers.has(compilerSettings)) {
      // console.log(new Date(), 'Adding compiler: ', compilerSettings);
    ...
    return function transformer(tree, file) {
      ...
      // console.log('Transpile counter: ', transpileCounter++);
      ...
      const result = flatMap(tree, function mapper(node: CodeNode) {
        if (node.type === 'code') {
          codeBlock++;
        }
...
${lines.slice(Math.max(0, diagnostic.line - 5), diagnostic.line + 6).join('\n')}
        ...
        const assembledResult = assembleReplacementNodes(
          node,
          file,
          virtualFolder,
          virtualFiles,
        ...
        // const currentCompilationTime = compilationEnd - compilationStart;
        // const currentAssemblyTime = assemblyEnd - assemblyStart;
        // compilationTime += currentCompilationTime;
        // assemblyTime += currentAssemblyTime;
      ...
      // console.log(new Date(), 'File compilation time: ', {
      //   virtualFilepath,
      //   compilationTime,
      //   assemblyTime,
      //   totalTime,
